<script lang="ts">
  ...
  let pluginResultMap = new Map<string, CommonListItem[]>();
  ...
  const setPluginResults = (e: CustomEvent) => {
    const { pluginPath, list } = e.detail || {}
    pluginResultMap = pluginResultMap.set(pluginPath, list)
  }

  const onResultEnter = (item: CommonListItem) => {
    if (!item) return;

    let targetPlugin: string = ''
    let targetIndex: number = 0
    for (const [pluginPath, list] of pluginResultMap) {
      const index = list.indexOf(item)
      if (index !== -1) {
        // 找到了
        targetIndex = index
        targetPlugin = pluginPath
        break;
      }
    }
